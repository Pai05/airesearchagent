export function createPaperCard(paper, onOpenSidebar) {
    ...
    const sourceText = paper.source.replace('_', ' ').toUpperCase();
    ...
    if (paper.authors && paper.authors.length > 0) {
        if (paper.authors.length > 2) {
            authorsText = `${paper.authors[0]}, ${paper.authors[1]} et al.`;
        } else {
    ...
    card.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
            <h3 class="card-title" style="margin: 0; font-size: 1.1rem; flex: 1;">${paper.title}</h3>
        ...
        <div style="display: flex; gap: 1rem; align-items: center; margin: 1rem 0; font-size: 0.75rem; color: var(--text-muted); font-weight: 600;">
            ...
            <span>•</span>
            <span class="badge" style="background: rgba(255,255,255,0.1); border: 1px solid var(--border);">${sourceText}</span>
            <span>•</span>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-top: 1.5rem;">
            <div>
                ...
                <div style="font-size: 0.875rem; color: var(--text-main); display: flex; flex-direction: column; gap: 0.5rem;">
                    ${paper.findings.length > 0 ? paper.findings.map(f => `<div>${f}</div>`).join('') : '<div style="color: var(--text-muted); font-style: italic;">No findings extracted</div>'}
                </div>
            </div>
            <div>
                ...
                <div style="font-size: 0.875rem; color: var(--text-main); display: flex; flex-direction: column; gap: 0.5rem;">
                    ${paper.gaps.length > 0 ? paper.gaps.map(g => `<div data-paper-id="${paper.id}">${g}</div>`).join('') : '<div style="color: var(--text-muted); font-style: italic;">No gaps identified</div>'}
                </div>
            </div>
        </div>
